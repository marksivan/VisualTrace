...
def build_execution_source(source, function_name, function_args):
...
def run_traced_code(source, stdin_data, function_name, function_args, enable_trace):
    old_stdin = sys.stdin
...
__vt_result_json__ = json.dumps(run_traced_code(
    __vt_source__,
    __vt_stdin__,
    __vt_function_name__ or None,
    __vt_function_args__,
    __vt_trace__,
))
`;
...
export async function executeInBrowser(
  ...
  pyodide.globals.set("__vt_function_args", request.function_args || []);
  pyodide.globals.set("__vt_trace", request.trace);

  await pyodide.runPythonAsync(TRACE_HARNESS);

  const output = await pyodide.runPythonAsync(`__vt_result_json__`);

  const parsed = JSON.parse(output as string) as ExecutionResult;
  return {
...
export const BROWSER_LANGUAGES = [
  { id: "python" as const, name: "Python", enabled: true },
  { id: "javascript" as const, name: "JavaScript", enabled: false },
  { id: "java" as const, name: "Java", enabled: false },
  { id: "cpp" as const, name: "C++", enabled: false },
];
